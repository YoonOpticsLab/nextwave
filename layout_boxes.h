...
struct shmem_boxes_header
{
  ...
  CALC_TYPE mirror_relax[MAX_MIRROR_VOLTAGES];
};

#define SHMEM_BUFFER_SIZE_BOXES 1+1+2+sizeof(CALC_TYPE)*3+4+MAX_BOXES*sizeof(CALC_TYPE)*10+4*MAX_BOXES+sizeof(CALC_TYPE)*MAX_TERMS*2+2*MAX_MIRROR_VOLTAGES*sizeof(CALC_TYPE)
